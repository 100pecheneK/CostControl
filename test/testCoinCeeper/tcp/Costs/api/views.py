...
class PostListAPIView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        if query:
            date = query
        queryset_list = Cost.objects.filter(date__icontains=date).order_by('-id')
...
class CostMonthSumAPIView(APIView):
    ...
    def get(self, request):
        ...
        if query:
            date = query
        query = Cost.objects.filter(user=request.user, date__icontains=date).aggregate(month_money_sum=Sum('money'))
...
class CostReportView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        if query:
            date = query
        query = Cost.objects.filter(date__icontains=date).values('cost_type').annotate(
